...
for (let i = 0; i < STREAK_COUNT; i++) {
  ...
  s.position.set(
    THREE.MathUtils.lerp(STREAK_MIN_X, STREAK_MAX_X, i / STREAK_COUNT),
    0.02,
...
const FOOD_Y = 1.65;
const FOOD_SPREAD_X = 0.85;
const FOOD_SPREAD_Z = 0.5;
...
loader.load(
  ...
  undefined,
  (err) => console.error('Помилка завантаження візка:', err)
);

const items = [];

function registerItem(obj, opts = {}) {
  obj.visible = false;
...
function createSimpleFoods() {
  const configs = [
    { geo: new THREE.SphereGeometry(0.22, 10, 10),          color: 0xff4444, offsetX: -0.20, offsetZ: -0.30 },
    { geo: new THREE.SphereGeometry(0.18, 10, 10),          color: 0xffcc00, offsetX:  0.55, offsetZ:  0.35 },
    { geo: new THREE.BoxGeometry(0.28, 0.35, 0.28),         color: 0x44aa44, offsetX:  0.70, offsetZ: -0.20 },
...
function prepModel(model, targetSize, rot) {
...
loader.load('./milk.glb', (gltf) => {
  const wrapper = prepModel(gltf.scene, 0.95, { x: -Math.PI / 2 });
...
}, undefined, (e) => console.error('Помилка завантаження молока:', e));

loader.load('./dandys_world_eggs.glb', (gltf) => {
  const wrapper = prepModel(gltf.scene, 0.9, null);
  registerItem(wrapper, {
    offsetX: 0.25,
    offsetZ: 0.25,
    landY: FOOD_Y + 0.1,
...
}, undefined, (e) => console.error('Помилка завантаження яєць:', e));

const startScreenEl = document.getElementById('startScreen');
const wipeLeftEl = document.getElementById('wipeLeft');
const wipeRightEl = document.getElementById('wipeRight');
const endScreenEl = document.getElementById('endScreen');
const endTextEl = document.getElementById('endText');

const END_PHRASES = ['ZLAGODA', 'Enter...'];
const TYPING_SPEED = 150;
const DELETING_SPEED = 150;
...
function startEndText() {
  ...
  const tick = () => {
    const phrase = END_PHRASES[phraseIdx];

    if (!deleting) {
      charIdx++;
      if (endTextEl) endTextEl.textContent = phrase.slice(0, charIdx);
      if (charIdx < phrase.length) {
        setTimeout(tick, TYPING_SPEED);
      } else {
        setTimeout(() => { deleting = true; tick(); }, PAUSE_DURATION);
      }
    } else {
      charIdx--;
      if (endTextEl) endTextEl.textContent = phrase.slice(0, charIdx);
      if (charIdx > 0) {
        ...
        deleting = false;
        phraseIdx = (phraseIdx + 1) % END_PHRASES.length;
        setTimeout(tick, 400);
      }
    }
  };
...
function enableStart() {
  ready = true;
  if (startScreenEl) startScreenEl.classList.add('ready');
}
...
window.addEventListener('pointerdown', beginIntro);
...
function setWipe(vw) {
  ...
  if (wipeLeftEl) {
    wipeLeftEl.style.width = w;
    wipeLeftEl.style.boxShadow = glow;
  }
  if (wipeRightEl) {
    wipeRightEl.style.width = w;
    wipeRightEl.style.boxShadow = glow;
  }
}
...
function startAnimation() {
  ...
  if (endTextEl) endTextEl.textContent = '';
...
function updateFalling(dt, cartX) {
  items.forEach((it) => {
